...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.wifiConnectAction = QAction('Reconnect WiFi', self)
        self.wifiConnectAction.triggered.connect(self.userWiFiReconnect)
        actionFile.addAction(self.wifiConnectAction)
        self.wifiConnectAction.setVisible(False)
        ...
        if self.readConfigFile():
            if self.commMethodInUse == "WiFi":
                ...
                self.wifiConnectAction.setVisible(True)
        ...
        self.checkBoxSparkGap.clicked.connect(self.handleCheckBoxSparkGap)
        ...
        self.lineEditSideTone.setText(str(self.sideTone))
        ...
        SST = SendSideTone(self.sideTone, self.sparkGap)
    ...
    def userWiFiReconnect(self):
        print("Reconnect WiFi")
        self.SerialComm.socketClose()
        self.initWiFiCommunications()
...
# ==== end of menu bar handlers
    ...
    def evalBool(self, B):
        try:
            ...
            print("Error! incorrect boolean: ", B)
            return False
    ...
    def loadPracticeSession(self):
        ...
        home_dir = ""
        Qtd = QFileDialog()
        Fname = Qtd.getOpenFileName(self, 'Load from saved session', home_dir, "Key Practice session files (*.cvs)")

        if Fname[0]:
            f = open(Fname[0], 'r')
            CsvFile = csv.reader(f)
            morseElementList = list(CsvFile)
            print(self.evalBool("True"))
            print(self.evalBool("true"))
            for savedElement in morseElementList:
                print(savedElement)
            """
            with f:
                data = f.read()
                self.morseTextEdit. setPlainText(data)
            """
        else:
            print("No file selected. pity.")
    ...
    def savePracticeSession(self):
        ...
        try:
            ...
            for morseChar in self.morseTextStream:
                ...
                if morseChar.getMorsePro() == 0xFF:
                    if morseChar.getValid():
                        saveSessionFile.write(morseChar.getMorseChar() + ", ")
                    else:
                        saveSessionFile.write("*, ")
                else:
                    saveSessionFile.write("\\"+self.prosignList.getPro(morseChar.getMorsePro())+", ")
                ...
                saveSessionFile.write(str(morseChar.getIdleAfter())) # Last value if prosign.
                """
                Prosign is value other than 0xFF.  For these, output element
                type and duration pairs.
                """
                if morseChar.getMorsePro() == 0xFF:
                    for element in morseChar.morseCharSeq:
                        saveSessionFile.write(", ")
                        saveSessionFile.write(str(element.morseElement.value) + ", ")
                        saveSessionFile.write(str(element.Duration))
                saveSessionFile.write("\n")
    ...
    def handleCheckBoxSparkGap(self):
        print("sparkGap")
        self.sparkGap = self.checkBoxSparkGap.isChecked()
        SST = SendSideTone(self.sideTone, self.sparkGap)
        self.msgXmitQueue.put(SST.getMsg())
        if self.sparkGap:
            self.lineEditSideTone.setEnabled(False)
            self.pushButtonSideTone.setDisabled(True)
        else:
            self.lineEditSideTone.setEnabled(True)
            self.pushButtonSideTone.setDisabled(False)

    def handlepushButtonSideTone(self):
        #print("pushButtonSideTone")
        self.sideTone = self.editToFloat(self.lineEditSideTone.text())
        self.sparkGap = False
        SST = SendSideTone(self.sideTone, self.sparkGap)
        self.msgXmitQueue.put(SST.getMsg())
